...
int main() {
    ...
    size_t const docsAmount = 800;
    size_t const wordsInDocument = 1000;
    size_t const queriesCount = 30000;
    size_t const wordsInQuery = 10;

    std::vector<string> docs(docsAmount);
    for (auto &doc : docs) {
        auto isFirst = true;
        for (auto i = 0; i < wordsInDocument; i++) {
            if (!isFirst) {
                doc += ' ';
            }
            doc += ("london" + std::to_string(i));
            isFirst = false;
        }
    }
    std::vector<string> queries(queriesCount);
    for (auto &query : queries) {
        auto isFirst = true;
        for (auto i = 0; i < wordsInQuery; i++) {
            if (!isFirst) {
                query += ' ';
            }
            query += ("london" + std::to_string(i));
            isFirst = false;
        }
    }
    SearchServer srv;
