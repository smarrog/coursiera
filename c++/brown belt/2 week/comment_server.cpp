...
std::ostream& operator << (std::ostream& output, HttpCode code) {
...
HttpResponse& HttpResponse::SetContent(std::string content) {
...
#define TEST_RESPONSE(expected, response) { \
    auto responseValue = response;          \
    std::stringstream ss;                   \
    ss << responseValue;                    \
    AssertEqual(expected, ss.str());       \
}

void CustomTest() {
    TEST_RESPONSE("HTTP/1.1 200 OK\n\n", HttpResponse(HttpCode::Ok));
    TEST_RESPONSE("HTTP/1.1 404 Not found\n\n", HttpResponse(HttpCode::NotFound));
    TEST_RESPONSE("HTTP/1.1 200 OK\nContent-Length: 3\n\nHi\n", HttpResponse(HttpCode::Ok).SetContent("Hi\n"));
    TEST_RESPONSE("HTTP/1.1 200 OK\nContent-Length: 14\n\nanother_string", HttpResponse(HttpCode::Ok).SetContent("string").SetContent("another_string"));
}
