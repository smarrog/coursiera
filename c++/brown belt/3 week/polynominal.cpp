...
private:
    ...
    public:
        ...
        IndexProxy& operator = (const T value) {
            if (_p.coeffs_.size() <= _degree) {
                if (value > 0) {
                    while (_p.coeffs_.size() <= _degree) {
                        _p.coeffs_.emplace_back(static_cast<T>(0));
                    }
                    _p.coeffs_[_degree] = value;
                }
            } else if (_p.coeffs_.size() == _degree + 1) {
                _p.coeffs_[_degree] = value;
                if (value == 0) {
                    _p.Shrink();
                }
        ...
        bool operator == (const T value) {
            return std::as_const(_p)[_degree] == value;
        }

        std::ostream& operator << (std::ostream& os) {
            return os << std::as_const(_p)[_degree];
        }

        operator T() const {
            return std::as_const(_p)[_degree];
        }
    private:
        Polynomial& _p;
        size_t _degree;
