...
class Budget {
public:
    Budget(double income = 0, double cost = 0)
        : _income(income)
        , _cost(cost) {}

    Budget operator + (const Budget& other) const {
        return { _income + other._income, _cost + other._cost };
    }
...
struct BulkTaxApplier {
    explicit BulkTaxApplier(int percent = 0)
            : factor(percent > 0 ? ((100.0 - percent) / 100.0) : 1.0) { }
...
public:
    ...
    BulkLinearUpdater(const BulkMoneyRemover& remove) : remove_(remove) {}
    BulkLinearUpdater(const BulkTaxApplier& tax) : tax_(tax) {}

    void CombineWith(const BulkLinearUpdater& other) {
        tax_.factor *= other.tax_.factor;
        add_.delta = other.tax_.factor * add_.delta + other.add_.delta;
        remove_.value += other.remove_.value;
    }

    Budget Collapse(Budget origin, IndexSegment segment) const {
        return { origin.getIncome() * tax_.factor + add_.delta * segment.length(), origin.getCost() + remove_.value * segment.length() };
    }

private:
    BulkTaxApplier tax_;
    BulkMoneyAdder add_;
    BulkMoneyRemover remove_;
};
...
struct ComputeIncomeRequest : ReadRequest<double> {
    ...
    double Process(const BudgetManager& manager) const override {
        return manager.ComputeSum(MakeDateSegment(date_from, date_to)).getTotal();
    }
...
struct SpendRequest : ModifyRequest {
    ...
    void Process(BudgetManager& manager) const override {
        ...
        manager.AddBulkOperation(date_segment, BulkMoneyRemover{daily_cost});
    }
